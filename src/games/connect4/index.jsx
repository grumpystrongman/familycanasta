...
import { chooseConnect4RobotMove, CONNECT4_RULES, createConnect4Game, reduceConnect4 } from "./engine";
import "./styles.css";
...
function Connect4Table({ controller }) {
  ...
  return (
    <main className="modular-game-shell connect4-shell"><section className="modular-game-panel connect4-table">
      ...
      <PlayerChips members={members} renderDetail={(member) => current?.uid === member.uid && state.phase === "playing" ? "Your move" : member.uid === state.winnerUid ? "Winner" : "Waiting"} />
      <div className="connect4-status"><strong>{state.message}</strong><span>{myTurn ? "Pick a column." : state.phase === "playing" ? `Waiting for ${current?.nickname}.` : "Round complete."}</span></div>
      <div className="connect4-board-wrap">
        <div className="connect4-drop-row" aria-label="Choose a column">
          {Array.from({ length: CONNECT4_RULES.columns }, (_, column) => (
            <button key={column} type="button" disabled={!myTurn || busy || state.board[column] != null} onClick={() => act({ type: "drop", column })} aria-label={`Drop in column ${column + 1}`}>▼</button>
          ))}
        </div>
        <div className="connect4-board" role="grid" aria-label="Connect 4 board">
          {(state.board || []).map((uid, index) => {
...
export default function Connect4Game() {
  ...
  if (controller.room.status === "lobby") return <GameLobby controller={controller} title="Connect 4" minimumPlayers={2} maxPlayers={2} startLabel="Drop the first checker" />;
