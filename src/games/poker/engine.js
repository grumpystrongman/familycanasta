...
function activeUids(state, members) { return members.map((member) => member.uid).filter((uid) => !state.folded?.[uid] && state.inHand?.[uid]); }
...
export function comparePokerHands(left, right) {
  ...
  for (let i = 0; i < length; i += 1) if ((a.tie[i] || 0) !== (b.tie[i] || 0)) return (a.tie[i] || 0) - (b.tie[i] || 0);
  return 0;
}

function beginBetting(state, phase, members) {
  const first = nextActiveIndex({ ...state, allIn: {} }, members, Number(state.dealerIndex || 0));
  return { ...state, phase, currentPlayerIndex: first, currentBet: 0, bettingContrib: Object.fromEntries(members.map((member) => [member.uid, 0])), acted: {}, raises: 0, allIn: {}, message: `${members[first].nickname} acts first.` };
}
...
function dealRound(members, roundNumber, previous = {}, random = Math.random) {
  ...
  const starting = Number(previous.startingPoints || POKER_RULES.startingPoints);
  const balances = { ...(previous.balances || Object.fromEntries(members.map((member) => [member.uid, starting]))) };
  ...
  return beginBetting({
    roundNumber, dealerIndex, hands, deck, discards: [], balances, pot, inHand,
    folded: {}, drawn: {}, revealed: false, winners: [], winnerUid: null,
    startingPoints: starting, ante, maxRaises: Number(previous.maxRaises || POKER_RULES.maxRaises),
    message: "First betting round.",
  }, "betting-1", members);
}
...
function pay(state, uid, amount) {
  const balances = { ...(state.balances || {}) };
  const available = Math.max(0, Number(balances[uid] || 0));
  const paid = Math.min(available, Math.max(0, amount));
  balances[uid] = available - paid;
  return { balances, paid, allIn: paid < amount || balances[uid] === 0 };
}
...
function bettingComplete(state, members) {
  ...
  return uids.every((uid) => state.allIn?.[uid] || (state.acted?.[uid] && Number(state.bettingContrib?.[uid] || 0) === Number(state.currentBet || 0)));
}
...
function finishBetting(state, members) {
  if (state.phase === "betting-1") return startDraw(state, members);
  return showdown(state, members);
}
...
function bet(state, actorUid, action, members) {
  ...
  const bettingContrib = { ...(state.bettingContrib || {}) };
  const allIn = { ...(state.allIn || {}) };
  let balances = { ...(state.balances || {}) };
  let pot = Number(state.pot || 0);
  let currentBet = Number(state.currentBet || 0);
  let raises = Number(state.raises || 0);

  if (action.move === "fold") {
    folded[actorUid] = true;
    acted[actorUid] = true;
    ...
    const target = currentBet + increment;
    const needed = target - Number(bettingContrib[actorUid] || 0);
    if (Number(balances[actorUid] || 0) < needed) throw new Error("Not enough points to raise. Call or fold instead.");
    const payment = pay({ balances }, actorUid, needed);
    balances = payment.balances; pot += payment.paid; bettingContrib[actorUid] = target; currentBet = target; raises += 1;
    Object.keys(acted).forEach((uid) => { acted[uid] = false; }); acted[actorUid] = true;
    if (payment.allIn) allIn[actorUid] = true;
  } else {
    const needed = Math.max(0, currentBet - Number(bettingContrib[actorUid] || 0));
    const payment = pay({ balances }, actorUid, needed);
    balances = payment.balances; pot += payment.paid; bettingContrib[actorUid] = Number(bettingContrib[actorUid] || 0) + payment.paid; acted[actorUid] = true;
    if (payment.allIn && payment.paid < needed) allIn[actorUid] = true;
  }

  let next = { ...state, folded, acted, bettingContrib, allIn, balances, pot, currentBet, raises };
  const uncontested = awardUncontested(next, members);
  if (uncontested) return uncontested;
  if (bettingComplete(next, members)) return finishBetting(next, members);
  next = { ...next, currentPlayerIndex: nextActiveIndex(next, members, Number(state.currentPlayerIndex || 0)) };
  return { ...next, message: `${members[next.currentPlayerIndex].nickname} acts next.` };
}
...
function drawCards(state, actorUid, cardIds, members) {
  ...
  const kept = hand.filter((card) => !selected.includes(card.id));
  while (kept.length < 5) kept.push(deck.shift());
  ...
  const remaining = activeUids(nextState, members).filter((uid) => !drawn[uid]);
  if (!remaining.length) return beginBetting(nextState, "betting-2", members);
  const nextIndex = nextActiveIndex({ ...nextState, allIn: {}, folded: { ...(nextState.folded || {}), ...Object.fromEntries(members.filter((member) => drawn[member.uid]).map((member) => [member.uid, true])) } }, members, Number(state.currentPlayerIndex || 0));
...
export function choosePokerRobotMove(state, members) {
  if (["betting-1","betting-2"].includes(state.phase)) {
    ...
    const hand = evaluatePokerHand(state.hands?.[active.uid] || []);
    const owed = Number(state.currentBet || 0) - Number(state.bettingContrib?.[active.uid] || 0);
    let move = "call";
    if (owed > 0 && hand.category === 0 && Number(state.balances?.[active.uid] || 0) > 5) move = "fold";
    else if (hand.category >= 2 && Number(state.raises || 0) < Number(state.maxRaises || POKER_RULES.maxRaises)) move = "raise";
