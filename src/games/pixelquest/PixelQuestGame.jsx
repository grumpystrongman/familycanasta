  ...
  return (
    <main className="pq-shell pq-title-screen">
      ...
      <section className="pq-title-hero">
        <p className="pq-overline">1–8 ONLINE ADVENTURERS · ONE PLAYER PER SCREEN · ORIGINAL d20 FANTASY</p>
        <h1><span>PIXEL</span>QUEST</h1>
        <h2>THE LIVING DUNGEON</h2>
        <p>An AI-DM-ready cooperative fantasy RPG where every family member joins from their own browser, owns one hero, rolls honest dice, and makes decisions that can change the party's story.</p>
      ...
      <section className="pq-portal-console">
        ...
        <div className="pq-feature-runes"><span>📱 One player, one screen</span><span>🎲 Visible dice</span><span>🗺 Tactical maps</span><span>🤖 AI companions</span><span>🗝 Private choices</span><span>🏆 Persistent legends</span></div>
      </section>
    </main>
  );
}
...
function PixelQuestLobby({ controller, audio }) {
  ...
  return (
    <main className="pq-shell pq-lobby-screen">
      ...
      <section className="pq-lobby-banner" style={{ "--quest-accent": adventure.accent }}><div><small>ROOM CODE</small><strong>{room.roomCode}</strong><p>Send this six-character code to up to seven more family members. Everyone joins from their own browser; nobody shares a game screen.</p></div><div className="pq-lobby-runes"><span>{adventure.tone}</span><span>{room.rules?.difficulty || "story"}</span><span>1–8 heroes</span></div></section>
      {error ? <p className="pq-error">{error}</p> : null}
...
function PartyHud({ campaign, members, seatHeroes, currentActor }) {
  return <aside className="pq-party-hud"><h3>THE PARTY</h3>{campaign.heroes.map((hero) => { const liveHero = campaign.combat?.actors?.find((actor) => actor.id === hero.id) || hero; const member = members.find((candidate) => seatHeroes?.[candidate.uid] === hero.id); const active = currentActor?.id === hero.id; return <article key={hero.id} className={`${active ? "active" : ""} ${liveHero.downed ? "downed" : ""}`}><PixelSprite heroId={hero.id} size="sm" /><div><strong>{member?.nickname || hero.name}</strong><small>{hero.name} · {hero.className}{member?.isRobot ? " · AI" : ""}</small><HealthBar hp={liveHero.hp} maxHp={liveHero.maxHp} compact /></div>{active ? <b>TURN</b> : null}</article>; })}</aside>;
}

function DMPanel({ campaign }) {
  const [showRolls, setShowRolls] = useState(false);
  const log = (campaign.log || []).slice(-8).reverse();
  return <aside className="pq-dm-panel"><div className="pq-dm-title"><span className="pq-dm-eye">◆</span><div><small>THE DUNGEON MASTER</small><strong>THE WORLD IS LISTENING</strong></div></div><div className="pq-dm-scroll">{log.map((entry) => <p key={entry.id} className={`type-${entry.type}`}>{entry.private ? "[An adventurer locked a private decision.]" : entry.text}</p>)}</div><button type="button" onClick={() => setShowRolls((value) => !value)}>🎲 {showRolls ? "Hide" : "Show"} dice history</button>{showRolls ? <div className="pq-roll-history">{(campaign.rollHistory || []).slice(-10).reverse().map((roll) => <span key={roll.id}><b>{roll.raw}</b><small>{roll.actorName}</small><em>{roll.kind} · {roll.outcome}</em></span>)}</div> : null}</aside>;
}

function StoryScene({ controller, campaign, scene, audio }) {
  const isHost = controller.room.hostUid === controller.user?.uid;
  return <section className="pq-story-stage"><div className="pq-scene-art scene-story"><div className="pq-pixel-landscape"><i className="tree t1" /><i className="tree t2" /><i className="road" /><i className="torch" /></div></div><div className="pq-story-copy"><span className="pq-scene-type">STORY</span><h1>{scene.title}</h1><p>{localNarrator.describe(campaign)}</p>{isHost ? <button type="button" className="pq-primary" disabled={controller.busy} onClick={() => { audio.tone([220, 277]); controller.act({ type: "continue-story" }); }}>CONTINUE →</button> : <p className="pq-waiting">Waiting for the party leader to continue…</p>}</div></section>;
}
...
function PartyDecision({ controller, campaign, scene, myHeroId, audio }) {
  ...
  function suggestPlan() {
    if (!plan.trim() || controller.busy) return;
  ...
  return <section className="pq-choice-stage"><div className="pq-choice-heading"><span className="pq-scene-type">PARTY DECISION</span><h1>{scene.title}</h1><p>{localNarrator.describe(campaign)}</p><small>Every adventurer votes independently from their own browser. Nobody can cast another player's vote.</small></div><div className="pq-choice-list">{scene.choices.map((choice, index) => { const voters = campaign.heroes.filter((hero) => campaign.votes?.[hero.id] === choice.id); return <button type="button" key={choice.id} disabled={!myHeroId || controller.busy} className={myVote === choice.id ? "chosen" : ""} onClick={() => { audio.tone([200 + index * 50]); controller.act({ type: "vote", choiceId: choice.id }); }}><b>{String.fromCharCode(65 + index)}</b><span><strong>{choice.label}</strong><small>{choice.detail}</small><i>{voters.map((hero) => <PixelSprite key={hero.id} heroId={hero.id} size="xs" />)}</i></span></button>; })}</div><div className="pq-free-plan"><label>DO SOMETHING ELSE<input value={plan} disabled={controller.busy} onChange={(event) => setPlan(event.target.value)} placeholder="I want to climb the roof and enter through the chimney…" /></label><button type="button" disabled={controller.busy} onClick={suggestPlan}>ASK THE DM</button>{dmAnswer ? <p>{dmAnswer}</p> : null}</div><footer><span>{myVote ? `Your vote: ${scene.choices.find((choice) => choice.id === myVote)?.label}` : "Choose your vote on this screen."}</span>{isHost && humanHeroIds.length > 1 ? <button type="button" className="pq-primary" disabled={!allHumanVotes || controller.busy} onClick={() => { audio.tone([250, 330, 500]); controller.act({ type: "resolve-vote" }); }}>{allHumanVotes ? "LOCK PARTY DECISION →" : "WAITING FOR VOTES"}</button> : null}</footer></section>;
}

function PrivateDecision({ controller, campaign, scene, myHeroId, audio }) {
  const [revealed, setRevealed] = useState(false);
  const myHero = HERO_BY_ID[myHeroId];
  const choiceKey = myHeroId ? `${scene.id}:${myHeroId}` : "";
  const myChoiceId = choiceKey ? campaign.privateChoices?.[choiceKey] : null;
  const myChoice = scene.choices?.find((choice) => choice.id === myChoiceId);
  const humanHeroIds = controller.members.filter((member) => !member.isRobot).map((member) => controller.room.gameState.seatHeroes?.[member.uid]).filter(Boolean);
  const waitingCount = humanHeroIds.filter((heroId) => !campaign.privateChoices?.[`${scene.id}:${heroId}`]).length;
  if (!myHeroId) return <section className="pq-private-stage"><h1>This secret belongs to an active adventurer.</h1></section>;
  if (myChoiceId) return <section className="pq-private-stage pq-private-wait"><PixelSprite heroId={myHeroId} size="xl" /><span className="pq-scene-type">YOUR DECISION IS LOCKED</span><h1>{myChoice?.label || "Choice recorded"}</h1><p>Your selection is stored under {myHero?.name}. Other players can see that you finished, but not which option you chose.</p><strong>{waitingCount > 0 ? `Waiting for ${waitingCount} other online adventurer${waitingCount === 1 ? "" : "s"}…` : "Synchronizing the party…"}</strong></section>;
  if (!revealed) return <section className="pq-private-stage veil"><PixelSprite heroId={myHeroId} size="xl" /><span className="pq-scene-type">PRIVATE MOMENT</span><h1>{myHero?.name}, this choice is yours.</h1><p>This decision is controlled only from your browser. Every other online player gets their own independent private turn on their own screen.</p><button type="button" className="pq-primary" disabled={controller.busy} onClick={() => { audio.tone([160, 220]); setRevealed(true); }}>REVEAL MY CHOICE</button></section>;
  return <section className="pq-private-stage"><span className="pq-scene-type">ONLY YOU DECIDE</span><h1>{scene.title}</h1><p>{scene.text}</p><div className="pq-private-choices">{scene.choices.map((choice) => <button type="button" key={choice.id} disabled={controller.busy} onClick={() => { audio.tone([220, 330]); controller.act({ type: "private-choice", choiceId: choice.id }); }}>{choice.label}</button>)}</div></section>;
}
...
function SkillScene({ controller, campaign, scene, myHeroId, audio }) {
  ...
  const attempts = campaign.skillAttempts?.[scene.id] || {};
  const myAttempt = attempts[myHeroId];
  const humanHeroIds = controller.members.filter((member) => !member.isRobot).map((member) => controller.room.gameState.seatHeroes?.[member.uid]).filter(Boolean);
  const completed = humanHeroIds.filter((heroId) => attempts[heroId]).length;
