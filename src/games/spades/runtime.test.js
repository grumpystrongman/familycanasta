...
test("Spades cannot be led before breaking when another suit is available", () => {
  ...
  assert.deepEqual(legalSpadesCards(state, "a", members).map((value) => value.id), ["4-clubs"]);
});
...
test("Spades requires following suit", () => {
  ...
  assert.deepEqual(legalSpadesCards(state, "a", members).map((value) => value.id), ["3-hearts"]);
});
...
test("a spade trumps every card of the led suit", () => {
  ...
  assert.equal(spadesTrickWinnerIndex(trick, members), 3);
});

test("Spades scores contracts, overtricks, and a successful nil", () => {
  const scored = scoreSpadesRound({
