  ...
  return (
    <main className="modular-game-shell golf-shell"><section className="modular-game-panel golf-table">
      ...
      <div className="golf-center">
        ...
        {state.drawnCard ? <div className="golf-drawn"><small>You drew</small><StandardCard card={state.drawnCard} disabled /><strong>Choose a grid card to replace.</strong>{state.drawnFrom === "stock" ? <button type="button" disabled={busy} onClick={() => act({ type: "discard-drawn" })}>Discard this card</button> : null}</div> : <p className="new-game-empty">Draw from the stock or visible discard, then decide whether it improves your six-card grid.</p>}
      </div>
