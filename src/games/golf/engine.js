...
function dealHole(members, holeNumber, previous = {}, random = Math.random) {
  ...
  return {
    phase: "reveal",
    holeNumber,
    roundNumber: holeNumber,
    dealerIndex,
    currentPlayerIndex: (dealerIndex + 1) % members.length,
    grids,
    faceUp: Object.fromEntries(members.map((member) => [member.uid, []])),
    revealReady: {},
    stock: deck.slice(1),
    discardPile: [deck[0]],
    drawnCard: null,
    ...
    holeScores: {},
    winnerUid: null,
    message: "Each player turns two grid cards face up.",
  };
}

export function createGolfGame(members, rules = {}, random = Math.random) {
  return { ...dealHole(members, 1, {}, random), holes: Number(rules.holes || GOLF_RULES.holes) };
}

function allPlayersRevealed(state, members) {
  return members.every((member) => (state.faceUp?.[member.uid] || []).length === 2);
}
...
function draw(state, actorUid, source, members, random = Math.random) {
  ...
  if (source === "discard") {
    ...
    ({ stock, discardPile } = replenishStock(stock, discardPile, random));
    card = stock.shift();
  }
...
function finishHole(state, members) {
  ...
  let winnerUid = null;
  if (finalHole) winnerUid = [...members].sort((a, b) => totals[a.uid] - totals[b.uid])[0]?.uid || null;
...
function swap(state, actorUid, index, members) {
  ...
  const grid = [...(grids[actorUid] || [])];
  const replaced = grid[index];
  grid[index] = state.drawnCard;
  grids[actorUid] = grid;
  const faceUp = { ...(state.faceUp || {}) };
  faceUp[actorUid] = [...new Set([...(faceUp[actorUid] || []), index])].sort((a, b) => a - b);
...
function discardDrawn(state, actorUid, members) {
  if (state.phase !== "playing" || !state.drawnCard) throw new Error("Draw a card first.");
  if (state.drawnFrom === "discard") throw new Error("A card taken from the discard pile must replace a grid card.");
...
export function chooseGolfRobotMove(state, members) {
  ...
  const grid = state.grids?.[active.uid] || [];
  const hidden = [0,1,2,3,4,5].filter((index) => !(state.faceUp?.[active.uid] || []).includes(index));
