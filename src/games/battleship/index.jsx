...
import { BATTLESHIP_FLEET, BATTLESHIP_RULES, cellLabel, chooseBattleshipRobotMove, createBattleshipGame, normalizeBattleshipFleet, reduceBattleship } from "./engine";
import "./styles.css";

function fleetCellMap(fleet = []) {
  const map = {};
  for (const ship of normalizeBattleshipFleet(fleet)) for (const cell of ship.cells) map[cell] = ship;
  return map;
}
...
function BattleshipTable({ controller }) {
  ...
  const myFleet = normalizeBattleshipFleet(state.fleets?.[user?.uid]);
  const enemyFleet = normalizeBattleshipFleet(state.fleets?.[opponent?.uid]);
  ...
  return (
    <main className="modular-game-shell battleship-shell"><section className="modular-game-panel battleship-table">
      ...
      <PlayerChips activeUid={state.phase === "playing" ? current?.uid : state.winnerUid} members={members} renderDetail={(member) => {
        const fleet = normalizeBattleshipFleet(state.fleets?.[member.uid]);
...
export default function BattleshipGame() {
  ...
  if (controller.room.status === "lobby") return <GameLobby controller={controller} title="Battleship" minimumPlayers={2} maxPlayers={2} startLabel="Deploy fleets" seatLabels={["Fleet commander", "Fleet commander"]} />;
